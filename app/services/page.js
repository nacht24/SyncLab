...
export default function Services() {
  return (
    <div className="max-w-6xl mx-auto py-16 md:py-24 px-4 sm:px-6">
      {/* Header Section */}
      <div className="text-center mb-12 md:mb-20">
        ...
        <h2 className="text-3xl sm:text-4xl md:text-5xl font-black mt-6 text-slate-50">
          Precision <span className="text-sync-teal">Services.</span>
        </h2>
        <p className="mt-4 text-slate-400 max-w-xl mx-auto italic text-sm sm:text-base">
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 lg:gap-8">
        {services.map((service, index) => (
          <div key={index} className="group p-6 sm:p-8 lg:p-10 bg-deep-surface rounded-2xl border border-slate-800/50 hover:border-sync-teal/50 transition-all duration-500 relative overflow-hidden">
            ...
            <h3 className="text-xl sm:text-2xl font-bold text-slate-50 mb-3 sm:mb-4 group-hover:text-sync-teal transition-colors">
              {service.title}
            </h3>
            ...
            <div className="mt-6 sm:mt-8 h-1 w-12 bg-slate-800 group-hover:w-full group-hover:bg-sync-teal transition-all duration-700"></div>
          </div>
        ))}
      </div>

      {/* Why Choose Us Section (Tambahan) */}
      <section className="mt-20 md:mt-32 pt-12 md:pt-20 border-t border-slate-900">
        <h2 className="text-2xl md:text-3xl font-bold mb-10 md:mb-16 text-center text-slate-50">
          Why Choose <span className="text-sync-teal">Sync</span><span className="text-slate-400">Lab</span>?
        </h2>
        <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-8 md:gap-12 text-left">
